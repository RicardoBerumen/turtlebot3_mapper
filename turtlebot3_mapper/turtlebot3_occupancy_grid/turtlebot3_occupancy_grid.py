...
class Turtlebot3OccupancyGrid(Node):
   ...
   def __init__(self, node_name: str = 'turtlebot_occupancy_grid'):
       ...
       self.declare_parameters(
           ...
           parameters=[
               (
                   "width", #Tamaño de ocupación del mapa
                   4.0,
                   ParameterDescriptor(description="Map's width"),
               ),
               (
                   "height",#Tamaño resolución del mapa
                   4.0,
                   ParameterDescriptor(description="Map's height"),
               ),
               (
                   "resolution",#Precisión del mapa
                   0.03,
                   ParameterDescriptor(description="Map's resolution"),
               ),
               (
                   "min_prob", #Probabilidad minima para definir si una celda esta ocupada o libre 
                   0.01,
                   ParameterDescriptor(description="Map's min probability value"),
               ),
               (
                   "max_prob", #Probabilidad maxima para definir si una celda esta ocupada o libre 
                   0.99,
                   ParameterDescriptor(description="Map's max probability value"),
               ),
               (
                   "prob_occupied", #Reduce los falsos positivos en detección de obstáculos
                   0.6,
                   ParameterDescriptor(description="Occupied probability increment"),
               ),
               (
                   "prob_free", #Ajusta la rapidez con la que el robot considera las celdas como ocupadas o libres.
                   0.4,
                   ParameterDescriptor(description="Free probability increment"),
               ),
               (
                   "prob_priori", #AJusta la probailidad incial antes de actualizar la ocupación
                   0.5,
   ...
   def update_map(self, message_laser: LaserScan):
       ...
       if not self._update.locked():
           self._update.acquire()
           # Establece un valor máximo para reemplazar los valores NaN en el escaneo LiDAR
           max_range = message_laser.range_max
          
           # Reemplaza valores NaN en los datos de rango del LiDAR con el valor máximo de rango
           clean_ranges = np.array(message_laser.ranges)
           clean_ranges[np.isnan(clean_ranges)] = max_range


           # Asigna los rangos limpiados al mensaje para continuar con el procesamiento
           message_laser.ranges = clean_ranges.tolist()
           ...
           # Convierte los datos de escaneo en coordenadas polares y luego a coordenadas cartesianas
   ...
   def update_cell(self, x: int, y: int, state: State):
       ...
       """
       """Actualiza la celda de la cuadrícula de ocupación según el estado observado."""
       # Verificar que x e y están dentro de los límites de la cuadrícula
       if x < 0 or y < 0 or x >= self.grid.shape[0] or y >= self.grid.shape[1]:
           return  # Salir de la función si los índices están fuera de los límites
