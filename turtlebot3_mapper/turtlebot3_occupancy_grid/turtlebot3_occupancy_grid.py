...
class Turtlebot3OccupancyGrid(Node):
    ...
    def update_map(self, message_laser: LaserScan):
        ...
        if not self._update.locked():
            ...
            if theta < 0.0:
                theta += 2 * np.pi
            polar = self.laser_scan_to_polar(message=message_laser)
            xy = self.polar_to_cartesian(
                coordinates=polar,
    ...
    def occupancy_grid(self):
        ...
        return self.numpy_to_occupancy_grid(
            array=grid,
    ...
    @staticmethod
    def polar_to_cartesian(coordinates: np.ndarray, x: float, y: float,
